...
if IS_BUNDLED:
    CODE_DIR = Path(sys._MEIPASS).resolve() if hasattr(sys, "_MEIPASS") else Path(sys.executable).parent.resolve()
    DATA_DIR = Path(os.environ.get("APPDATA", Path.home() / "AppData" / "Roaming")) / "Feedscript" if IS_WINDOWS else Path.home() / "Library" / "Application Support" / "Feedscript"
else:
    CODE_DIR = Path(os.environ.get("FEEDSCRIPT_CODE_DIR", Path(__file__).parent)).resolve()
    DATA_DIR = Path(os.environ.get("FEEDSCRIPT_DATA_DIR", Path(__file__).parent)).resolve()

DATA_DIR.mkdir(parents=True, exist_ok=True)
os.environ["FEEDSCRIPT_CODE_DIR"] = str(CODE_DIR)
os.environ["FEEDSCRIPT_DATA_DIR"] = str(DATA_DIR)

ROOT = DATA_DIR  # legacy alias — anything that needs the working dir
VENV_PY = DATA_DIR / "venv" / ("Scripts" if IS_WINDOWS else "bin") / ("python.exe" if IS_WINDOWS else "python3")
REQS = CODE_DIR / "requirements.txt"
SETUP_HTML = CODE_DIR / "templates" / "setup.html"
PORT = 8765
        ...
        if IS_BUNDLED:
            ...
            try:
                self.server_proc = subprocess.Popen(
                    ...
                    cwd=str(CODE_DIR),
                    env={**os.environ, "FEEDSCRIPT_CODE_DIR": str(CODE_DIR), "FEEDSCRIPT_DATA_DIR": str(DATA_DIR)},
